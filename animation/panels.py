...
class MapGraph:
    ...
    def display(self, world: MapCanvas):
        ...
        self.y_floor = self.y_px + self.h_px - (2*world.style.font_px)
        self.bar_h = self.h_px - (4*world.style.font_px)
        # Create y index lines
        self.index_lines = []
        self.index_labels = []
        for i in range(3):
            y = self.y_floor - (self.bar_h*i / 2)
            p1 = (self.x_px + self.x_frac - int(world.style.font_px/2), y)
            p2 = (self.x_px + self.w_px - self.x_frac + int(world.style.font_px/2), y)
            line = world.canvas.create_line(p1, p2, fill=world.style.text, width=1)
            self.index_lines.append(line)
            text = "%.0f" % (self.max_height*i / 2)
            label = world.canvas.create_text(self.x_px + self.x_frac - world.style.font_px, y, text=text, fill=world.style.text, anchor=tkinter.E)
            self.index_labels.append(label)

        # Create bar labels
        self.bar_labels = []
        for i in range(len(self.bar_names)):
            label = world.canvas.create_text(self.x_px + (self.x_frac*(2 + 3*i)), self.y_floor + world.style.font_px, text=self.bar_names[i], fill=world.style.text)
            self.bar_labels.append(label)
        self._draw_bars(world)

    def _draw_bars(self, world):
        self.bars = []
